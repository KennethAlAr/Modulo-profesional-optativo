...
def anadir_nota_alumno(alumno, nota):
    if alumno not in notas_alumnos.keys():
        raise ValueError(f"El alumno {alumno} no existe en el sistema.")
...
def eliminar_nota_alumno(alumno, nota):
    if nota not in notas_alumnos[alumno]:
        raise ValueError(f"La nota {nota} no se encuentra en el registro de notas.")
...
def main():
    ...
    while opcion != "6":
        match opcion:
            case "1":
                try:
                    alumno = input("¿Qué media quieres consultar?\n")
                    print(f"{consultar_media(alumno)}")
            ...
            case "2":
                try:
                    ...
                    anadir_nota_alumno(alumno, nota)
                except Exception as e:
                    print(e)
            case "3":
                try:
                    notas = []
                    alumno = input("¿Cúal es el nombre del alumno?\n")
            ...
            case "4":
                try:
                    ...
                    print(f"Las notas del alumno {alumno} son {mostrar_notas_alumno(alumno)}")
                    nota = float(input("¿Qué nota quieres eliminar?\n"))
